...
function ShowPaplayer(furl,width,height,autostart,startpic,baseurl)
 {
    ...
    str+='var so = new SWFObject("/Scripts/paplayer.swf","ply","'+width+'","'+height+'","9","#000000");so.addParam("allowfullscreen","true");so.addParam("allowscriptaccess","always");so.addParam("wmode","opaque");so.addParam("quality","high");so.addParam("salign","lt");so.addVariable("JcScpFile","/Scripts/set.xml");';
